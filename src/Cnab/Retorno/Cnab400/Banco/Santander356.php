...
class Santander356 extends AbstractRetorno implements RetornoCnab400
{
    /**
     * Código do banco
     *
    ...
    private $ocorrencias = [
        ...
        '04' => 'Baixado e Creditado em C. Corrente',
        '05' => 'Liquidação de título não registrado',
    ...
    protected function processarHeader(array $header)
    {
        $this->getHeader()
            ...
            ->setConvenio($this->rem(34, 39, $header))
    ...
    protected function processarDetalhe(array $detalhe)
    {
        ...
        $d = $this->detalheAtual();
        $d->setNossoNumero($this->rem(127, 134, $detalhe))
            ->setNumeroDocumento($this->rem(117, 126, $detalhe))
            ->setNumeroControle($this->rem(38, 62, $detalhe))
            ->setOcorrencia($this->rem(109, 110, $detalhe))
            ->setOcorrenciaDescricao(array_get($this->ocorrencias, $d->getOcorrencia(), $d->getOcorrencia() . ' - Desconhecida'))
        ...
        $valorMulta = Util::nFloat($this->rem(215, 227, $detalhe) / 100, 2, FALSE);
        $valorOutrosCreditos = Util::nFloat($this->rem(280, 292, $detalhe) / 100, 2, FALSE);
        
        $d->setValorMulta($valorMulta + $valorOutrosCreditos);
        
        $this->totais['valor_recebido'] += $d->getValorRecebido();

        if ($d->hasOcorrencia('06', '07', '08', '17')) {
            ...
            $this->totais['erros']++;
            $d->setError(array_get($this->rejeicoes, $this->rem(340, 341, $detalhe), ''));
        } else {
